...
public class AFD {
	...
	/**
	 * Nesse m�todo est� implemtado o algoritmo para gera��o de L(M), sendo M um
	 * AFD
	 */
	public void gera(Estado e, String p) {
		if (getEstadosFinais().pertence(e)) {
			System.out.println(p);
		}

		ConjuntoTransicaoD ct = getFuncaoPrograma();
		TransicaoD t = null;

		if (ct.vazio() == false) {
			for (Iterator iter = ct.iterator(); iter.hasNext();) {
				TransicaoD tAux = (TransicaoD) iter.next();
		...
		while (t != null) {
			...
			if (t.getOrigem().igual(e)) {
				...
				if (aux.length() > 10) {
					...
					// continua compondo a palavra a partir do
					// estado atingido
					gera(t.getDestino(), aux);
			}
			if (ct.vazio() == false) {
				for (Iterator iter = ct.iterator(); iter.hasNext();) {
					TransicaoD tAux = (TransicaoD) iter.next();
